...
function doPost(
  ...
  if (eventType !== "reaction_added" && eventType !== "reaction_removed") {
    return ContentService.createTextOutput("Not reaction event");
  }
  ...
  const originalMessageDate = formatDate(date);
  ...
  if (eventType === "reaction_added") {
    ...
    // reaction_removed の場合、該当するレコードを削除
    ...
    logToSheet("Remove End", { userId, originalMessageDate });
  }
